...
try {
    ...
    $mail->Username   = 'uralprokat@createers.tech'; // Логин на почте
    $mail->Password   = '7y0peKwx;KJR'; // Пароль на почте
    $mail->SMTPSecure = 'ssl';
    $mail->Port       = 465;
    $mail->setFrom('uralprokat@createers.tech', 'URALPROCAT96'); // Адрес самой почты и имя отправителя
    // Получатель письм 
...
/*if (!empty($_FILES['myfile']['name'][0])) {
    for ($ct = 0; $ct < count($_FILES['myfile']['tmp_name']); $ct++) {
        ...
        Примечание: <b>Бeсплатная доставка</b>";
